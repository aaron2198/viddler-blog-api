...
func (aq *ArticleQuery) QueryUserOptions() *UserOptionsQuery {
	query := (&UserOptionsClient{config: aq.config}).Query()
...
func (aq *ArticleQuery) Clone() *ArticleQuery {
	...
	return &ArticleQuery{
		...
		withUserOptions: aq.withUserOptions.Clone(),
...
func (aq *ArticleQuery) WithUserOptions(opts ...func(*UserOptionsQuery)) *ArticleQuery {
	query := (&UserOptionsClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withUserOptions = query
	return aq
}
...
func (aq *ArticleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Article, error) {
	var (
		...
		_spec       = aq.querySpec()
		loadedTypes = [1]bool{
			aq.withUserOptions != nil,
		}
	)
	if aq.withUserOptions != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withUserOptions; query != nil {
		if err := aq.loadUserOptions(ctx, query, nodes, nil,
			func(n *Article, e *UserOptions) { n.Edges.UserOptions = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ArticleQuery) loadUserOptions(ctx context.Context, query *UserOptionsQuery, nodes []*Article, init func(*Article), assign func(*Article, *UserOptions)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Article)
	for i := range nodes {
		if nodes[i].user_options_article == nil {
			continue
		}
		fk := *nodes[i].user_options_article
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_options_article" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
